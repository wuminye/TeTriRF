...
class DirectVoxGO(torch.nn.Module):
    def __init__(self, xyz_min, xyz_max,
        ...
        self.density = grid.create_grid(
                ...
                xyz_min=self.xyz_min, xyz_max=self.xyz_max, residual_mode=residual_mode,
        ...
        if rgbnet_dim <= 0:
            ...
            self.k0 = grid.create_grid(
                    k0_type, channels=self.k0_dim, world_size=self.world_size,
                    xyz_min=self.xyz_min, xyz_max=self.xyz_max, residual_mode = self.residual_mode,
                    config=self.k0_config)
    ...
    def update_mask_cache_from_file(self, mask_cache_path):
        mask_cache_world_size = self.world_size
        if mask_cache_path is not None and mask_cache_path:
            mask_cache = grid.MaskGrid(
                    path=mask_cache_path,
    ...
    def scale_volume_grid(self, num_voxels):
        ...
        #    raise Exception("residual_mode not allow volume scaling")
    ...
    def update_occupancy_cache(self):
        ...
        print('**update occupancy cache, percentage:', (self.mask_cache.mask.sum().float() / self.mask_cache.mask.numel()).item() * 100, '%')
        return (self.mask_cache.mask.sum().float() / self.mask_cache.mask.numel()).item() * 100

    @torch.no_grad()
    def reset_occupancy_cache(self):
        cache_grid_xyz = torch.stack(torch.meshgrid(
            torch.linspace(self.xyz_min[0], self.xyz_max[0], self.mask_cache.mask.shape[0]),
            torch.linspace(self.xyz_min[1], self.xyz_max[1], self.mask_cache.mask.shape[1]),
            torch.linspace(self.xyz_min[2], self.xyz_max[2], self.mask_cache.mask.shape[2]),
    ...
    def forward(self, rays_o, rays_d, viewdirs, shared_rgbnet=None, global_step=None, mode='feat', **render_kwargs):
        '''Volume rendering
        ...
        if self.rgbnet_full_implicit:
            ...
            if self.k0_dim>3:
                center = (self.xyz_max + self.xyz_min)/2
                #k0 = self.k0(ray_pts,dir = viewdirs[ray_id])
                k0 = self.k0(ray_pts,center = center)
            else:
                k0 = self.k0(ray_pts)
        ...
        '''
        density = self.densitynet(k0).squeeze(-1)
        alpha = self.activate_density(density, interval)
        #print(ray_pts.size(), k0.size(),alpha.size())
        weights, alphainv_last = Alphas2Weights.apply(alpha, ray_id, N)
        '''

        if shared_rgbnet is None:
            ...
            if mode =='feat':
                feature_marched = segment_coo(
                    src=(weights.unsqueeze(-1) * k0),
                    index=ray_id,
                    out=torch.zeros([N, k0.size(1)]),
                    reduce='sum')

                if self.rgb_model =='MLP':

                    viewdirs_emb = (viewdirs.unsqueeze(-1) * self.viewfreq).flatten(-2)
                    viewdirs_emb = torch.cat([viewdirs, viewdirs_emb.sin(), viewdirs_emb.cos()], -1)

                    rgb_feat = torch.cat([feature_marched, viewdirs_emb], -1)

                    rgb_logit = shared_rgbnet(rgb_feat)
                    rgb = torch.sigmoid(rgb_logit)
                elif self.rgb_model =='SH':
                    rgb = shared_rgbnet(feature_marched, dirs = viewdirs)
            else:
                if self.rgb_model =='MLP':
                    viewdirs_emb = (viewdirs.unsqueeze(-1) * self.viewfreq).flatten(-2)
                    viewdirs_emb = torch.cat([viewdirs, viewdirs_emb.sin(), viewdirs_emb.cos()], -1)
                    ...
                    rgb = shared_rgbnet(k0_view, dirs = viewdirs)
               
            

        if mode =='feat':
            rgb_marched = (alphainv_last.unsqueeze(-1) * render_kwargs['bg']) + rgb
        else:

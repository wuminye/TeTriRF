...
class DirectMPIGO(torch.nn.Module):
    ...
    def scale_volume_grid(self, num_voxels):
        ...
        if np.prod(self.world_size.tolist()) <= 256**3:
            ...
            old_mask = self.mask_cache(self_grid_xyz)
            temp = self.mask_cache(self_grid_xyz) & (self_alpha > self.fast_color_thres)
            new_p = temp.float().mean().item()
            if new_p > 0.36:
                old_mask = temp
            else:
    ...
    def update_occupancy_cache(self):
        ...
        cache_grid_density = (self.density(cache_grid_xyz)+ self.act_shift(cache_grid_xyz))[None,None]
        cache_grid_alpha = self.activate_density(cache_grid_density)
        cache_grid_alpha = F.max_pool3d(cache_grid_alpha, kernel_size=3, padding=1, stride=1)[0,0]
        #self.mask_cache.mask &= (cache_grid_alpha > self.fast_color_thres)

        temp = self.mask_cache.mask & (cache_grid_alpha > self.fast_color_thres)
        new_p = temp.float().mean().item()
        if new_p > 0.16:
            self.mask_cache.mask = temp
        else:
            print('DISCARD UPDATE.')
        print(f'dmpigo: update mask_cache {ori_p:.4f} => {new_p:.4f}  {(self.mask_cache.mask.sum().float() / self.mask_cache.mask.numel()).item() * 100}%')
    ...
    def forward(self, rays_o, rays_d, viewdirs, shared_rgbnet=None, global_step=None, mode='feat', **render_kwargs):
        ...
        if shared_rgbnet is None:
            ...
            if mode =='feat':
                feature_marched = segment_coo(
                    src=(weights.unsqueeze(-1) * vox_emb),
                    index=ray_id,
                    out=torch.zeros([N, vox_emb.size(1)]),
                    reduce='sum')
        ...
        if mode =='feat':
            if render_kwargs.get('rand_bkgd', False) and global_step is not None:
                rgb_marched = (alphainv_last.unsqueeze(-1) * torch.rand_like(rgb)) + rgb
            else:
